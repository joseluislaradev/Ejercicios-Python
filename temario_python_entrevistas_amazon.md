# Temario de Python para entrevistas técnicas y LeetCode
...
La clave es ver que el * repite los elementos de la lista sobre la que está actuando.
...
```python
rows = len(matrix) #Numero de elemntos de la matriz, numero de arrays dentro
cols = len(matrix[0]) #dentro del primer array, cuantos elemntos tiene
...
`defaultdict` sirve cuando quieres evitar estar preguntando si una clave existe. Sirve para crear un diccionario de listas facil.
